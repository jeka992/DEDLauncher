...
public class MainViewModel : BaseViewModel
{
    ...
    private void StartFriends()
    {
        ...
        _friends.FriendKeysReceived += OnFriendKeysReceived;

        _ = _friends.StartAsync();

        foreach (var f in Friends.ToList())
        {
            if (!string.IsNullOrEmpty(f.SignPub) && !string.IsNullOrEmpty(f.AgreePub))
                _friends.SetFriendKey(f.Code, f.SignPub, f.AgreePub);
    ...
    private void OnRequestReceived(string code, string name, string signPub, string agreePub)
    {
        Application.Current.Dispatcher.Invoke(() =>
        {
            if (Friends.Any(f => f.Code == code)) return;
            if (FriendRequests.Any(r => r.Code == code)) return;
            FriendRequests.Add(new FriendEntry { Code = code, Name = name, SignPub = signPub, AgreePub = agreePub });
            OnPropertyChanged(nameof(HasFriendRequests));
            ShowToast($"Заявка в друзья от {(string.IsNullOrEmpty(name) ? code : name)}");
        });
    }

    private void OnRequestAccepted(string code, string name, string signPub, string agreePub)
    {
        Application.Current.Dispatcher.Invoke(() =>
        {
            if (Friends.Any(f => f.Code == code)) return;
            var friend = new FriendEntry { Code = code, Name = name, SignPub = signPub, AgreePub = agreePub };
            Friends.Add(friend);
            SaveFriends();
            _friends?.SetFriendKey(code, signPub, agreePub);
            _ = _friends!.AddFriendAsync(code);
            ShowToast($"{(string.IsNullOrEmpty(name) ? code : name)} принял(а) заявку ✓");
        });
    }

    private void OnFriendKeysReceived(string code, string signPub, string agreePub)
    {
        Application.Current.Dispatcher.Invoke(() =>
        {
            var friend = Friends.FirstOrDefault(f => f.Code == code);
            if (friend == null) return;
            if (friend.SignPub != signPub || friend.AgreePub != agreePub)
            {
                friend.SignPub = signPub;
                friend.AgreePub = agreePub;
                SaveFriends();
            }
        });
    }
    ...
    public async Task AcceptRequestAsync(FriendEntry request)
    {
        ...
        var friend = new FriendEntry
        {
            ...
            AgreePub = request.AgreePub
        };
        Friends.Add(friend);
        SaveFriends();
        StartFriends();
        if (!string.IsNullOrEmpty(friend.SignPub) && !string.IsNullOrEmpty(friend.AgreePub))
            _friends!.SetFriendKey(friend.Code, friend.SignPub, friend.AgreePub);
...
// ─── Launch History ───
    ...
    private double _todayMinutes;
    private double _totalMinutes;
    public double TodayMinutes { get => _todayMinutes; set { SetProperty(ref _todayMinutes, value); OnPropertyChanged(nameof(TodayTimeLabel)); } }
    public double TotalMinutes { get => _totalMinutes; set { SetProperty(ref _totalMinutes, value); OnPropertyChanged(nameof(TotalTimeLabel)); } }
    public string TodayTimeLabel => FormatMinutesLong(TodayMinutes);
    public string TotalTimeLabel => FormatMinutesLong(TotalMinutes);

    private static string FormatMinutesLong(double m)
    {
        var h = (int)(m / 60); var min = (int)(m % 60);
        ...
        try
        {
            if (File.Exists(AccountsFilePath))
            {
                ...
                if (list != null)
                    ...
                    {
                        a.AccessToken = TokenProtection.Unprotect(a.AccessToken);
                        a.RefreshToken = TokenProtection.Unprotect(a.RefreshToken);
                        Accounts.Add(a);
                    }
            }
        }
    ...
    private void SaveAccounts()
    {
        try
        {
            var list = Accounts.Select(a => new AccountInfo
            {
                Username = a.Username,
                Uuid = a.Uuid,
                AccessToken = TokenProtection.Protect(a.AccessToken),
                RefreshToken = TokenProtection.Protect(a.RefreshToken),
                ExpiresAt = a.ExpiresAt,
                AccountType = a.AccountType,
                SkinPath = a.SkinPath,
                AvatarUrl = a.AvatarUrl
            }).ToList();
            File.WriteAllText(AccountsFilePath, JsonSerializer.Serialize(list, new JsonSerializerOptions { WriteIndented = true }));
        }
        catch { }
    }
    ...
    private void LoadSavedAccount()
    {
        try
        {
            ...
            if (File.Exists(p))
            {
                ...
                Account.AccessToken = TokenProtection.Unprotect(Account.AccessToken);
                Account.RefreshToken = TokenProtection.Unprotect(Account.RefreshToken);
    ...
    {
        var copy = new AccountInfo
        {
            Username = Account.Username,
            Uuid = Account.Uuid,
            AccessToken = TokenProtection.Protect(Account.AccessToken),
            RefreshToken = TokenProtection.Protect(Account.RefreshToken),
    ...
    private DateTime _todayKey;

    /// <summary>Фиксирует время сессии в статистику и сохраняет её.</summary>
    private void FinishGameStats()
    {
        try
        {
            if (_gameStopwatch != null)
            {
                ...
                if (minutes > 0.05) // больше ~3 секунд
                {
    ...
    private void SavePlayStats()
    {
        try
        {
            var path = Path.Combine(MinecraftPathHelper.BaseDir, "playstats.json");
            File.WriteAllText(path, JsonSerializer.Serialize(new
            {
                total = TotalMinutes,
                day = _todayKey.ToString("yyyy-MM-dd"),
                dayMinutes = _sessionTodayBefore
            }));
        }
        catch { }
    }
    ...
    private void LoadPlayStats()
    {
        try
        {
            ...
            TotalMinutes = root.TryGetProperty("total", out var t) ? t.GetDouble() : 0;
            if (root.TryGetProperty("day", out var d) && root.TryGetProperty("dayMinutes", out var dm))
            {
                if (DateTime.TryParse(d.GetString(), out var day))
                {
                    _todayKey = day.Date;
                    _sessionTodayBefore = dm.GetDouble();
                    if (_todayKey.Date != DateTime.Now.Date)
                    {
                        _todayKey = DateTime.Now.Date;
                        _sessionTodayBefore = 0;
                        TodayMinutes = 0;
                    }
                    else TodayMinutes = _sessionTodayBefore;
                }
            }
        }
        catch { }
    ...
    public async Task SearchModsAsync()
    {
        ...
        try
        {
            ...
            {
                if (!await _mods.IsModrinthReachableAsync())
                {
                    Status = "Modrinth заблокирован в РФ — переключаю на CurseForge";
                    ModSource = "CurseForge";
                    var cfResults = await _mods.SearchCurseForgeAsync(ModSearchQuery, CurrentProfile.VersionId);
                    _allCfMods = cfResults;
                    _modCfOffset = 0;
                    CurseForgeResults.Clear();
                    foreach (var r in cfResults.Take(CfChunkSize)) CurseForgeResults.Add(r);
                    _modCfOffset = CurseForgeResults.Count;
                    OnPropertyChanged(nameof(HasMoreCfMods));
                    _ = LoadCurseForgeIconsAsync(CurseForgeResults.ToList());
                    Status = CurseForgeResults.Count > 0
                        ? $"Найдено модов (CurseForge): {cfResults.Count}"
                        : $"По запросу «{ModSearchQuery}» ничего не найдено";
                    return;
                }
    ...
    private async Task LoadRecommendedResourcePacksAsync()
    {
        ...
        try
        {
            if (!await _mods.IsModrinthReachableAsync())
            {
                Status = "Modrinth заблокирован в РФ — переключаю на CurseForge";
                RpSource = "CurseForge";
                await LoadCurseForgeResourcePacksAsync();
                return;
            }
    ...
    private async Task LoadRecommendedShadersAsync()
    {
        ...
        try
        {
            ...
            {
                Status = "Modrinth заблокирован в РФ — переключаю на CurseForge";
                ShaderSource = "CurseForge";
                await LoadCurseForgeShadersAsync();
                return;
            }
...
public class FriendEntry : INotifyPropertyChanged
{
    ...
    public string SignPub { get; set; } = "";
    public string AgreePub { get; set; } = "";
